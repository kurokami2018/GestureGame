...
public class VideoCaptureModule {
	...
	public VideoCaptureModule(String key) {
		...
		if(m_capture.isOpened())
		{
			...
			{
				//カメラパラメータの取得
				int imageWidth = (int)m_capture.get(Videoio.CV_CAP_PROP_FRAME_WIDTH); //画像の幅
				int imageHeight = (int)m_capture.get(Videoio.CV_CAP_PROP_FRAME_HEIGHT); //画像の高さ
				int fourcc = (int)m_capture.get(Videoio.CV_CAP_PROP_FOURCC); //コーデック
				int fps = (int)m_capture.get(Videoio.CV_CAP_PROP_FPS); //フレームレート
				m_writer = new VideoWriter("test.mp4",
